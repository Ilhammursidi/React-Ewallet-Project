import { useLocation, NavLink, useParams, useNavigate } from "react-router";
...
import { transfer } from "../../Redux/slice/register";
...
export const UserTransfer = () => {
    const money = useSelector((state) => state.auth.currentUser)
    ...
    const handleSubmit = (e) => {
        ...
        if (userPin === pinStr) {
            if(money?.balance < amount) {
                ...
                setIsOpen(false)
                setIsOpen3(true)
                return toast.error("saldo tidak cukup")
            }
